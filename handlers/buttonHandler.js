...
async function handleButtonInteraction(interaction, activeGames, client, dealCardsWithDelay) {
    ...
    // Handle side bet buttons
    if (['perfect_pairs_bet', 'insurance_bet', 'start_dealing'].includes(customId)) {
        await handleSideBetButtons(interaction, activeGames, client, dealCardsWithDelay);
        return;
    }
...
async function handleSideBetButtons(interaction, activeGames, client, dealCardsWithDelay) {
    ...
    if (customId === 'start_dealing') {
        ...
        game.startDealing();
        await interaction.deferUpdate();
        await dealCardsWithDelay(interaction, interaction.message, game, user.id, 1000);
        return;
    }
...
module.exports = { handleButtonInteraction, handleBlackjackButtons, handleSideBetButtons, handleTableButtons, updateBettingDisplay, startTurnTimer };